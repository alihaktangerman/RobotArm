...
class triplenum:
    def __init__(self, a, b , c):
        ...
        self.c = c
    def __neg__(self):
        return triplenum(-self.a, -self.b, -self.c)
    def __add__(self, other):
        return triplenum(self.a+other.a, self.b+other.b, self.c+other.c)
    def __sub__(self, other):
        return self+(-other)
    def __mul__(self, other):
        return triplenum(self.a*other.a, self.a*other.b + self.b*other.a, self.a*other.c + 2*self.b*other.b + self.c*other.a)
    def __pow__(self, n):
        ret = triplenum(1,0,0)
        for i in range(n):
            ret = ret*self
    ...
    def __truediv__(self, other):
        assert(other.b == 0)
        return triplenum(self.a/other.a, self.b/other.a, self.c/other.a)
    def __str__(self):
        return f'{self.a} + {self.b}*eps + {self.c}eps^2'
"""
class triplenummat:
    def __init__(self, arr):
        self.arr = arr
    def __matmul__(self, other):
        arr = [[triplenum(0,0) for i in range(sle)] for j in range(3)]
        for i in range(3):
            for j in range(3):
                arr[i][j] = sum(self.arr[i][k]*other.arr[k][j] for k in range(3))
        return triplenummat(arr)
    def __str__(self):
...
def cos(x):
	...
	if x.a > 2*pi:
		x.a %= 2*pi
	return triplenum(1,0,0) - x**2/f(2) + x**4/f(4) - x**6/f(6) + x**8/f(8) - x**10/f(10) + x**12/f(12) - x**14/f(14) + x**16/f(16) - x**18/f(18) + x**20/f(20) - x**22/f(22) 
...
def rotmatrix3(theta, axis):
    r2 = rotmatrix2(theta)
    ret = [[triplenum(0,0,0) for _ in range(3)] for _ in range(3)]
    for i, ii in enumerate([(axis-1)%3,(axis+1)%3]):
        for j, jj in enumerate([(axis-1)%3,(axis+1)%3]):
            ret[ii][jj] = r2[i][j]
    ret[axis][axis] = triplenum(1,0,0)
    if axis==1:
        ret[0][2] = -ret[0][2]
        ret[2][0] = -ret[2][0]
    return np.array(ret)
...
def eyematrix3():
    return [[triplenum(1 if i==j else 0, 0) for i in range(3)] for j in range(3)]
"""
class affinepoin:
...
class roboarm6dof:
	def __init__(self, a, r, o):
		...
		self.a = a
		assert len(r) == 6
		self.r = [triplenum(x,0,0) for x in r]
		self.o = [triplenum(x,0,0) for x in o]
	def o0(self,t):
...
class projectedroboarm6dof:
    ...
    def a0(self,t):
        ret = self.roboarm.p0(t)
        ret = self.ps(np.array([x.c for x in ret]))
        return pygame.math.Vector2([x for x in ret])
    def a1(self,t):
        ret = self.roboarm.p1(t)
        ret = self.ps(np.array([x.c for x in ret]))
...
while time()-t < 500:
    ...
    #pygame.draw.line(screen, linecolor, vec+my_pra.p0((tnow-t)/20), vec+my_pra.p1((tnow-t)/20), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p1((tnow-t)/20), vec+my_pra.p2((tnow-t)/20), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p2((tnow-t)/20), vec+my_pra.p3((tnow-t)/20), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p3((tnow-t)/20), vec+my_pra.p4((tnow-t)/20), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p4((tnow-t)/20), vec+my_pra.p5((tnow-t)/20), width=5)
    pygame.draw.line(screen, linecolor, vec, vec+my_pra.p6((tnow-t)/20), width=2)

    #pygame.draw.line(screen, linecolor, vec+my_pra.p0((tnow-t)/20), vec+my_pra.p0((tnow-t)/20)+my_pra.v0(t), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p1((tnow-t)/20), vec+my_pra.p1((tnow-t)/20)+my_pra.v1(t), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p2((tnow-t)/20), vec+my_pra.p2((tnow-t)/20)+my_pra.v2(t), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p3((tnow-t)/20), vec+my_pra.p3((tnow-t)/20)+my_pra.v3(t), width=5)
    #pygame.draw.line(screen, linecolor, vec+my_pra.p4((tnow-t)/20), vec+my_pra.p4((tnow-t)/20)+my_pra.v4(t), width=5)
    pygame.draw.line(screen, vellineco, vec+my_pra.p6((tnow-t)/20), vec+my_pra.p6((tnow-t)/20)+my_pra.v6((tnow-t)/20), width=2)
    pygame.draw.line(screen, acclineco, vec+my_pra.p6((tnow-t)/20), vec+my_pra.p6((tnow-t)/20)+my_pra.a6((tnow-t)/20), width=2)
    pygame.display.flip()
